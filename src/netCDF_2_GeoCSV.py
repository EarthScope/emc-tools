...
'''
 NAME: netCDF_2_GeoCSV_3D.py - a Python script to read a 3D netCDF Earth model file and convert it to GeoCSV format.
...
def get_variable_attributes(model_data, header, variable, variable_name):
    ...
    header.append(f'# {variable_name.replace("_", "-")}_dimensions: {len(model_data.variables[variable].shape)}\n')

    for attr, value in vars(model_data.variables[variable]).items():
        if '_range' in attr:
            header.append(f'# {variable_name.replace("_", "-")}_{attr}: {value[0]},{value[1]}\n')
        else:
...
def get_model_header(model_file, model_data):
    ...
    header.append(f'# created: {datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")} UTC ({SCRIPT})\n')
    ...
    if VIEW_HEADER and (X_VARIABLE is None or Y_VARIABLE is None):
        ...
        header = get_variable_attributes(model_data, header, Y_VARIABLE, 'y')#, Y_VARIABLE)
        header = get_variable_attributes(model_data, header, X_VARIABLE, 'x')#, X_VARIABLE)
        if ndim == 3:
            header = get_variable_attributes(model_data, header, Z_VARIABLE, 'z')#, Z_VARIABLE)
...
def check_netcdf_file():
    ...
    # could not find the file
        ...
        print(f"[ERR] could not find the netCDF model file {NETCDF_FILE_NAME}", flush=True)
        usage()
        sys.exit(1)
        ...
        for i, this_lat in enumerate(y):
            ...
            for j, this_lon in enumerate(x):
                ...
                for var_index, var_value in enumerate(list(model_data.variables)):
                    ...
                    if var.encode('ascii', 'ignore').decode("utf-8") not in [X_VARIABLE, Y_VARIABLE,
                                                                             Z_VARIABLE]:
