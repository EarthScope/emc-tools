...
'''
 NAME: GeoCSV_2_netCDF_2D.py - a Python script to read a 2D GeoCSV Earth model file and convert it to netCDF format.
...
def set_dimensions(this_dataset, header_params, latitude_list, longitude_list):
...
def create_2d_variables(this_dataset, this_params, data, latitude_list, longitude_list):
    """ create 2D variables
    ...
    for var in var_list:
        ...
        var_values[var] = np.empty((len(latitude_list), len(longitude_list)))
    ...
    for line in data:
        ...
        for var in var_list:
            ...
            var_values[var][lat_index, lon_index] = value
...
lat_list, lon_list = get_dimensions(data_columns, params)
...
dataset, latitudes, longitudes = create_coordinate_variables(dataset, params)
