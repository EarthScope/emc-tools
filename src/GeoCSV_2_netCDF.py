...
def read_geocsv(file_name):
    ...
    for line in lines:
        ...
        # Data lines.
            ...
            data.append(
                list(str2float(line.split(header_params["delimiter"]["value"])))
            )
...
def set_dimensions(
    this_dataset, header_params, x_coord_list, y_coord_list, z_values_list
):
    ...
    if header_params["max_dimensions"]["value"] > 2:
        z_dim = this_dataset.createDimension(
            header_params["z"]["column"], len(z_values_list)
        )

    y_dim = this_dataset.createDimension(
        header_params["y"]["column"], len(y_coord_list)
    )
    x_dim = this_dataset.createDimension(
        header_params["x"]["column"], len(x_coord_list)
    )
...
def create_coordinate_variables(ds, par):
    ...
    if this_value:
        ...
        coordinate[y_column] = ds.createVariable(y_column, VAR_DTYPE, (y_column,))
    ...
    if this_value:
        ...
        coordinate[x_column] = ds.createVariable(x_column, VAR_DTYPE, (x_column,))
...
def check_geocsv_file():
    ...
    if geocsv_file_name is None:
        ...
        usage()
        print(f"[ERR] geocsv_file_name: {geocsv_file_name}")
        sys.exit(1)
    ...
    # could not find the file
        ...
        print(
            f"\n[ERR] could not find the GeoCSV model file {geocsv_file_name}",
            flush=True,
        )
        usage()
        sys.exit(1)
...
try:
    options, remainder = getopt.getopt(
        sys.argv[1:], "hHi:d", ["help", "input=", "debug", "header"]
    )
